...
/*
 * THIS IS ONLY FOR REFERENCE FROM THE QoL SOURCE, DELETE WHEN DONE WITH THE MODMENU
 */
    ...
    public static class ModMenu
    {
        ...
        private static List<IMenuMod.MenuEntry> GetModuleToggleMenuData()
        {
            ...
            (
                ...
                Bools,
                string.Empty,
                i => ++i, //QoL.ToggleModule(name, Convert.ToBoolean(i)),
                () => Convert.ToInt32(32) //QoL.GlobalSettings.EnabledModules[name]
            );

            return _TogglableModuleNames
                   .Where(x => false)//!SettingsOverride.TryGetModuleOverride(x, out _))
                   .Select(CreateEntry)
                   .ToList();
        }
